...
var inboundApiDownlink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_inbound_api_downlink",
                Help: "",
        },
)
...
var inboundMetricsInDownlink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_inbound_metrics_in_downlink",
                Help: "",
        },
)
...
var inboundMetricsInUplink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_inbound_metrics_in_uplink",
                Help: "",
        },
)
...
var inboundVlessTlsDownlink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_inbound_vless_tls_downlink",
                Help: "",
        },
)
...
var inboundVlessTlsUplink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_inbound_vless_tls_uplink",
                Help: "",
        },
)
...
var outboundBlockDownlink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_outbound_block_downlink",
                Help: "",
        },
)
...
var outboundBlockUplink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_outbound_block_uplink",
                Help: "",
        },
)
...
var outboundDirectDownlink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_outbound_direct_downlink",
                Help: "",
        },
)
...
var outboundDirectUplink = promauto.NewGauge(
        prometheus.GaugeOpts{
                Name: "xray_outbound_direct_uplink",
                Help: "",
        },
)
...
func updateStats(url string, wait int) {
      go func() {
              for {
                        stats, err := loadXrayStats(url)

                        if err != nil {
                                log.Println("Error polling XRay endpoint, trying again in", wait, "sec")
                        } else {
...
func loadXrayStats(url string) (stats *StatsRoot, err error) {
...
func rootHandler(w http.ResponseWriter, r *http.Request) {
        w.Write([]byte("Hello to XRay exporter"))
}

func main() {
        urlPtr := flag.String("url", "", "XRay status endpoint (normally: http://host/debug/vars)")
        ...
        waitPtr := flag.Int("interval", 15, "Interval to update statistics from XRay")
        ...
        log.Printf("Starting XRay Exporter on :%v%s\n", *portPtr, *endpointPtr)
